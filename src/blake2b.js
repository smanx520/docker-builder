// BLAKE2b（纯 JS，兼容 Cloudflare Workers）。
// 移植自 blakejs（https://github.com/dcposch/blakejs，MIT），与 GitHub 官方 tweetsodium 使用的实现一致。
// 仅保留 seal 需要的 blake2bInit / blake2bUpdate / blake2bFinal 及一次性 blake2b。

// 64 位加法：v[a,a+1] += v[b,b+1]（v 为 Uint32Array，用两个 32 位表示一个 64 位）
...
// 64 位加法：v[a,a+1] += b（b0 低 32 位，b1 高 32 位）
...
// 创建 BLAKE2b 上下文（outlen: 输出字节数 1~64）
...
// 一次性计算 BLAKE2b：返回长度为 outlen 的 Uint8Array
...
export { blake2bInit, blake2bUpdate, blake2bFinal, blake2b };
